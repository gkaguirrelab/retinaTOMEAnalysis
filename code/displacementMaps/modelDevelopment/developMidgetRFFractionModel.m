...
for mm = 1:length(p.Results.meridianAngles)
    ...
    % Fit a spline to the RF density data
    coneDensityFit = fit(nativeSupportPosDeg',coneDensitySqDeg','smoothingspline', 'Exclude',find(isnan(coneDensitySqDeg)),'SmoothingParam', 1);
    
    % Obtain the cumulative cone function
    cone_ringcount = calcCumulative(regularSupportPosDeg,coneDensityFit(regularSupportPosDeg)');
    ...
    % Calculate a proportion of the cumulative cone density counts, relative
    % to the reference point (which is assigned a value of unity)
    propCone_ringcount=coneDensityFit(regularSupportPosDeg)'./max(coneDensityFit(regularSupportPosDeg)');
    ...
    zeroPoints=find(propCone_ringcount==0);
    if ~isempty(zeroPoints)
        propCone_ringcount(zeroPoints)=min(propCone_ringcount(find(propCone_ringcount~=0)));
    end
    
    % calculate the mRF density at these eccentricity locations using
    % Watson equation 8.
    [ midgetRFDensitySqDeg ] = getWatsonMidgetReceptiveFieldDensityByEccen(regularSupportPosDeg, p.Results.meridianAngles(mm));

    % obtain the cumulative mRF density function
    mRF_ringcount = calcCumulative(regularSupportPosDeg,midgetRFDensitySqDeg);
    
    % calculate the ratio of mRFs to cones at each support position
    mRFtoConeRatioByEccen = midgetRFDensitySqDeg./coneDensityFit(regularSupportPosDeg)';
...
%     weights=ones(1,length(propCone_ringcount));
%     weights(1)=1e5;
%     
...
%     recipFit= ...
%         fit(log10(propCone_ringcount'),p.Results.watsonEq8_f0-midgetFractionByEccen', ...
...
%     fitParams(mm,3)=recipFit.c;
    ...
    if p.Results.makePlots
        plot((propCone_ringcount),(mRFtoConeRatioByEccen),'.','color',[.8 .8 .8])
        hold on
%        ylim([0 3]);
