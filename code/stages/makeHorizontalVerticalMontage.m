...
for e = 1:2 %eye side
    for i = 1:length(AllSubIDs) %subject folders found.
        ...
        subDir = fullfile(inputVolandSegDir,AllSubIDs(i).name,subject.eyeSide);
        

        saveName = fullfile(thicknessMapSaveDir,num2str(subject.ID),[num2str(subject.ID) '_' subject.eyeSide '.mat']);
        ...
        if(length(subVolFiles) ~= 2 )% check there is one H and one V file
           ...
           subject.BothAbsThicknessDiffOnSLOInterp = BothAbsDiff;
            ...
            mkdir(fullfile(thicknessMapSaveDir,num2str(subject.ID)));
