...
h=regressionPlot(comboTable.Axial_Length_average, gcMedianThick_reconstruct', 'Axial Length [mm]','Median GC Tissue Volume [mm^3 / deg^2]', [],1);
setTightFig
saveas(h,fullfile(saveDir,'fig8','b.png'));

% Plot GC thickness vs axial length, ['Axial length vs. median GC thickness, r=',num2str(corr(comboTable.Axial_Length_average,comboTable.gcMeanThick))]
h=regressionPlot(comboTable.Axial_Length_average, gcMeanThick_reconstruct', 'Axial Length [mm]','Mean GC Tissue Volume [mm^3 / deg^2]', [],1);
setTightFig
saveas(h,fullfile(saveDir,'fig8','c.png'));

% Plot GC thickness vs axial length, ['Axial length vs. median GC thickness, r=',num2str(corr(comboTable.Axial_Length_average,comboTable.gcMeanThick))]
h=regressionPlot(comboTable.Axial_Length_average, gcMeanThick_reconstruct_orig', 'Axial Length [mm]','Mean GC Tissue Volume [mm^3 / deg^2]', [],1);
setTightFig
